...
class Block(torch.nn.Module):#残差块
    def __init__(self, inputC, outputC, strides):
        ...
        if strides != 1 or inputC != outputC:#原图和经过残差块处理后维度不同
            self.shortcut = torch.nn.Sequential(
                #torch.nn.Conv2d(inputC, outputC, kernel_size=1, stride=strides, padding=1, bias=False),
                torch.nn.Conv2d(inputC, outputC, kernel_size=1, stride=strides, bias=False),
                torch.nn.BatchNorm2d(outputC),
            )
    def forward(self, x):
        output = self.block(x)
...
with open("./Data/train_class_acc.txt", 'w') as f3:
    for i in range(len(train_class_acc)):
        f3.write(str(train_class_acc[i]))
        f3.write('\n')
    f3.flush()
    f3.close()
